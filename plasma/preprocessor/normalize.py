    ...
    def apply(self,shot):
        ...
        if self.is_inference:
            ...
            if self.conf['data']['augment_during_training']:
                to_augment = random.sample(shot.signals)
            else:
                to_augment = None 
        if to_augment is not None:
            shot.signals_dict[to_augment] = self.augment(shot.signals_dict[to_augment])

    def augment(self,sig):
        print ('is augmenting signal'.format(sig))
        if self.conf['data']['augmentation_mode'] == "noise":
            return randn(sig.shape) #if noise augmentation
        elif self.conf['data']['augmentation_mode'] == "zero":
            return sig*0 #if "set to zero" augmentation. Can control in conf.
        elif self.conf['data']['augmentation_mode'] == "none":
            return sig #if no augmentation. Should be the default in conf.
        else:
