...
class Target(object):
    ...
    def loss_np(y_true,y_pred):
        import plasma.conf
        return plasma.conf.conf['model']['loss_scale_factor']*mse_np(y_true,y_pred)
...
class BinaryTarget(Target):
    ...
    def loss_np(y_true,y_pred):
        import plasma.conf 
        return plasma.conf.conf['model']['loss_scale_factor']*binary_crossentropy_np(y_true,y_pred)
...
class MaxHingeTarget(Target):
    ...
    def loss(y_true, y_pred):
        ...
        return plasma.conf.conf['model']['loss_scale_factor']*overall_fac*weight_mask*hinge(y_true,y_pred1)
    ...
    def loss_np(y_true, y_pred):
        ...
        return plasma.conf.conf['model']['loss_scale_factor']*np.mean(overall_fac*weight_mask*np.maximum(1. - y_true * y_pred, 0.))#, axis=-1) only during training, here we want to completely sum up over all instances
...
class HingeTarget(Target):
    ...
    def loss_np(y_true, y_pred):
        import plasma.conf
        return plasma.conf.conf['model']['loss_scale_factor']*hinge_np(y_true,y_pred)
