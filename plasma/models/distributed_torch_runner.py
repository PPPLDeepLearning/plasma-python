...
def train_epoch(model, data_gen, optimizer, scheduler, loss_fn):
    ...
    #         print(y.shape,output.shape)
        ...
        print("[{}]  [{}/{}] loss: {:.3f}, ave_loss: {:.3f}".format(
            step, num_so_far - num_so_far_start, num_total, loss.item(),
            total_loss/step))
...
def hvd_average_scalars(val, name):
...
def train(conf, shot_list_train, shot_list_validate, loader):
    ...
    if conf['callbacks']['mode'] == 'max':
        ...
        best_so_far = np.inf
        cmp_fn = min
    optim = opt.Adam(train_model.parameters(), lr=lr)
    scheduler = opt.lr_scheduler.ExponentialLR(optim, lr_decay)
